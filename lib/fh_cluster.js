...
var boundTasks = require('./bound_tasks.js');
 ...
 */
module.exports = function fhCluster(workerFunc, optionalNumWorkers, optionalBackoffStrategy, boundWorkerTasks, gracefullShutdownFn) {
  ...
  if (cluster.isMaster) {
    ...
    startWorker( workerFunc, gracefullShutdownFn );
  }
};

/********************************************/
/* MASTER - code that powers master process */
/********************************************/
...
function startMaster(numWorkers, backoffStrategy, backoffResetTimeout) {
  ...
  // set up master process event handlers
  cluster.on('exit', setupOnExitBackOff(backoffStrategy, backoffResetTimeout));
  cluster.on('exit', setupBoundsTasks);
  cluster.on('listening', setOnWorkerListeningHandler);
...
function setupOnExitBackOff(strategy, backoffResetTimeout) {
  return function(worker) {
    if (worker.suicide) {
      log.debug('Worker #', worker.id, 'commited suicide, won\'t retry');
      return;
    }
    var nextRetry = strategy.next();
    log.info('Worker #', worker.id, 'exited. Will retry in',
      nextRetry, 'ms');
...
function setOnWorkerListeningHandler(worker, address) {
  var host = address.address || address.addressType === 4 ? '0.0.0.0' : '::';
  var addr = host + ':' + address.port;
  log.info('Cluster worker',  worker.id, 'is now listening at', addr);
...
function cleanMasterShutdown(type) {
  return function (cb) {
    log.debug('exiting master with ' + type + ' [' + process.pid + ']');
    eachWorker(function(worker) {
      worker.send('shutdown');
    });
    waitForAllDead(function() {
      log.debug('exited master with ' + type + ' [' + process.pid + ']');
      if (cb) return cb();
      process.exit(0);
    });
  }
...
function waitForAllDead(cb) {
  ...
  eachWorker(function(worker) {
    if (!isWorkerDead(worker)) {
      log.debug('worker ' + worker.id + ' not dead');
      someAlive = true;
      setTimeout(function() {
        waitForAllDead(cb);
      }, 50);
...
function isWorkerDead(worker ) {
  return ('isDead' in worker) ? worker.isDead() : worker.state === 'dead';
}
...
/**********************************************/
/* WORKER - code that powers worker processes */
/**********************************************/

function startWorker(workerFunc, gracefullShutdownFn) {
  ...
  process.on('message', function(msg) {
    if (msg === 'shutdown') {
      cleanWorkerShutdown('shutdown')(gracefullShutdownFn);
    }
  });
...
function cleanWorkerShutdown(type) {
  return function (cb) {
    log.debug('exiting worker ' + cluster.worker.id + ' with ' + type + ' [' + process.pid + ']');
    if ( cb ) return cb( exitWorker );
    exitWorker();
  }
}
...
/******************************************************/
/* COMMON - code that powers both, master and workers */
/******************************************************/
